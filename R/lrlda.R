lrlda <-
function (Xtrain, group, Xtest = NULL, divisorn = FALSE, verbose = FALSE) 
{
    ...
    if(divisorn) {
      ...
      for (i in 1:ng) {
        Sp <- Sp + ((gsizes[i] - 1)/(sum(gsizes)-ng))* S.list[[i]]
      }  
    }
    if (verbose) {
    ...
    pred <- rep(NA, nrow(LD))
    for (i in 1:nrow(LD)) {
        Dif <- rep(1, ng) %o% LD[i, ] - Fp
        dsq <- diag(Dif %*% t(Dif)) - log(w)
        pred[i] <- levels(d$group)[which.min(dsq)]
    } # classification is distance and prevalence based 
    CM <- table(group, pred)
    if (verbose) {
    ...
    if (!is.null(Xtest)) {
        ...
        for (i in 1:nrow(LD.test)) {
          Dif <- rep(1, ng) %o% LD.test[i, ] - Fp
          dsq <- diag(Dif %*% t(Dif)) - log(w)
          pred.test[i] <- levels(d$group)[which.min(dsq)]
        } 
        
        prob.posterior.test <- matrix(0, nrow = nrow(LD.test), ncol = ng)
    
        for (i in 1:ng) {
          if (ncol(LD.test) > 1) {
             prob.posterior.test[, i] <- dens.mvnormSisI(LD.test, m = Mld[i,])
          }
          else {
        ...
        for (i in 1:ng) {
          PostTot <- PostTot + w[i] * prob.posterior.test[, i]
        }
        for (i in 1:ng) {
          prob.posterior.test[, i] <- w[i] * prob.posterior.test[, i]/PostTot
        }
